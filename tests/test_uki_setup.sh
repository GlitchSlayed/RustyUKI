...
grep -q 'DRY_RUN=0' "$BUILD_SCRIPT"
grep -q 'EFI_AUDIT_LOG="/var/log/uki-efi-audit.log"' "$BUILD_SCRIPT"
grep -q 'backup_efibootmgr_snapshot()' "$BUILD_SCRIPT"
grep -q 'prune_boot_entries_by_label()' "$BUILD_SCRIPT"
grep -q 'bootorder_has_alternate_entry()' "$BUILD_SCRIPT"
grep -q 'EFI registration validation failed' "$BUILD_SCRIPT"
grep -q 'Usage: uki-build.sh \[--dry-run\] \[--reconcile\] \[kernel-version\]' "$BUILD_SCRIPT"
grep -q '"\$0" --dry-run "\$kernel_ver"' "$BUILD_SCRIPT"
